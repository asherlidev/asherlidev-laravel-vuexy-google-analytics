<template>
  <div class="vehicle-page">
        ...
        <div v-if="showAddToCart" class="vehicle-page__add-to-cart">
          ...
          <div v-else-if="!productLoading && product">
            ...
            <SvgViewer
              ...
              :width="100"
              :height="100"
              :svg-id="currentClickedTooltip.calloutNumber"
            />
            <MProductAddToCart
    ...
    <div class="vehicle-page__footer">
      <div class="vehicle-page__footer-title">
        <h3>Your Vehicle</h3>
      </div>
      <p>{{ description }}</p>
    </div>
...
import { SfImage, SfLoader, SfSticky } from '@storefront-ui/vue';
...
export default {
  ...
  computed: {
    ...mapGetters({
      ...
      currentHoverTooltip: 'vehicles/currentSvgCodeTooltip',
      svgEventName: 'vehicles/svgEventName'
    }),
    description () {
  ...
  methods: {
    ...
    async clickParentPartItem (partItem) {
      this.selectedParentPartItem = partItem;
      if (!partItem.value) {
        this.selectedPartItem = partItem.child[0];
        await this.fetchTooltips(this.selectedPartItem.value);
      } else {
        await this.fetchTooltips(partItem.value);
      }
    },
    async clickPartItem (partItem) {
    ...
    async fetchTooltips (visual_category) {
      ...
      } = await axios.post('http://localhost:8080/api/tooltip', {
        NATIONAL_CODE: this.nationalCode,
        VISUAL_CATEGORY: visual_category
      });

      this.loading = false;
      this.imageId = imageId ? String(imageId) : null;
...
.vehicle-page {
  ...
  &__info {
    ...
    box-shadow: rgba(0, 0, 0, 0.1) 0px 8px 32px 0px,
      rgba(0, 0, 0, 0.05) 0px 16px 48px 0px;
    background: #fff;
    flex: 1;
