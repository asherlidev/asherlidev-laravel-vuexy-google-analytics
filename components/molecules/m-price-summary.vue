...
export default {
  ...
  computed: {
    ...
    totalFittingPrice () {
      return this.productsInCart.reduce((result, product) => {
        return this.getFittingPrice(product)
      }, 0)
    }
  },
  methods: {
    ...
    getFittingPrice (product) {
      return (
        // '£' +
        Number(LocationStorage.getFittingPrice(
          this.getAttributeLabelById('brand', product.brand),
          product.fitting_time
        ))
      );
    }
  }
